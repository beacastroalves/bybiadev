...
- ✅ **Parallax dos blobs** — FEITO (2.ª tentativa). A 1.ª (ScrollTrigger `scrub` no mesmo contexto dos reveals) e a 2.ª (hook `useParallax` separado) falharam: a segunda dava **quebra de ordem de hooks** (2 hooks a mexer no mesmo `scope`) que crashava o componente. **Solução:** o parallax vive **dentro do `useSiteReveals`** (um só hook), via **listener de scroll + `gsap.quickTo`** (NÃO ScrollTrigger). Sem hook extra, sem colisão de triggers. Verificado no browser: blob translada ~±18% suave ao rolar (ex.: -12px → +11px) e os reveals continuam intactos. *(Nota: testes JS com scroll instantâneo davam falsos negativos; a verificação boa é com scroll real.)*
